...
class SyncEmployeeToDevices implements ShouldQueue
{
    ...
    public function handle(MQTTClient $mqttClient, TenantDatabaseManager $manager): void
    {
        try {
            ...
            // Step 4: Get all active devices for this tenant
            $devices = Device::on('tenant')
                ->where('is_active', true)
                ->get();

            if ($devices->isEmpty()) {
                Log::channel('mqtt')->info('No active devices found for sync', [
                    'employee_id' => $this->employeeId,
                    'tenant_id' => $this->tenantId,
                ]);
            ...
            // Step 5: Sync employee to each device
            foreach ($devices as $device) {
                try {
                    // Check if enrollment exists for updates
                    ...
                    // Get photo (may be empty)
                    $photo = $this->getEmployeePhotoBase64($employee);

                    // Prepare MQTT payload for AddPerson or EditPerson command
     ...
     *
     * Resizes image to max 800x800px and optimizes for MQTT transmission
     */
